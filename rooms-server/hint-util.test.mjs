...
ok(
  u.leaksAnswer('KALEM', 'Üçüncü harf ünlü, K ile başlayan bir kelime dene') === false,
  'temiz ipucu sızıntı DEĞİL',
);
ok(u.leaksAnswer('MASA', 'Sarı harfleri başka yere kaydır') === false, 'genel ipucu sızıntı değil');

// --- validateInput ---
const good = u.validateInput({
  length: 5,
  lang: 'tr',
  answer: 'kalem',
...
ok(
  u.validateInput({ length: 5, answer: 'kalem', guesses: [{ word: 'ab', pattern: '00' }] })
    .error === 'bad_guess_word',
  'yanlış uzunlukta tahmin reddedilir',
);
ok(
  u.validateInput({ length: 5, answer: 'kalem', guesses: [{ word: 'araba', pattern: '00X00' }] })
    .error === 'bad_pattern',
  'bozuk desen reddedilir',
);
ok(u.validateInput({ length: 9, answer: 'x', guesses: [] }).error, 'geçersiz uzunluk reddedilir');
ok(
  u.validateInput({ length: 5, answer: 'kalem', guesses: [] }).error === 'bad_guesses',
  'boş tahmin listesi reddedilir',
);

// --- sanitizeHint: XML/etiket temizliği ---
ok(
  u.sanitizeHint('<thinking>gizli</thinking> Gerçek ipucu') === 'gizli Gerçek ipucu' ||
    !u.sanitizeHint('<thinking>x</thinking> ipucu').includes('<'),
  'XML etiketleri temizlenir',
);

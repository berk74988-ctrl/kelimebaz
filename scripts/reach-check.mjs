  ...
  return page.evaluate(() => {
    ...
    const els = [
      ...document.querySelectorAll(
        'main button, main input, main label, main a, .modal button, .modal input',
      ),
    ].filter(vis);
    ...
    for (const e of els) {
      ...
      if (r.bottom > ih + 2 || r.top < -2) {
        bad.push({
          txt: (e.getAttribute('aria-label') || e.textContent || e.tagName).trim().slice(0, 22),
...
async function check(name, width, height, nav) {
  ...
  try {
    ...
    console.log(`  ✗ ${name} nav: ${e.message}`);
    await ctx.close();
    fail++;
    return;
  }
  ...
  if (!ok) fail++;
  console.log(
    `  ${ok ? '✓' : '✗'} ${name} ${width}×${height} — ${r.total} öğe · ${ok ? 'hepsi erişilebilir' : `${r.bad.length} ERİŞİLEMEZ: ${r.bad.map((b) => b.txt + '(alt ' + b.bottom + '/' + r.ih + ')').join(', ')}`}`,
  );
  await ctx.close();
}
...
const nav = {
  ...
  'profil-görev': async (p) => {
    await tool(p, 1);
    await p.getByRole('button', { name: /Görevler/ }).click();
  },
  'profil-avatar': async (p) => {
    await tool(p, 1);
    await p.getByRole('button', { name: /Avatar/ }).click();
  },
  'magaza-avatar': async (p) => {
    await tool(p, 0);
    await p.locator('.tab', { hasText: 'Avatar' }).click();
  },
  ayarlar: async (p) => {
    await tool(p, 2);
  },
...
console.log(
  fail === 0
    ? '✅ tüm ekranlarda her buton/bilgi erişilebilir\n'

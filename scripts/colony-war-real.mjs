...
for (const [name, data] of Object.entries(SCEN)) {
  ...
  if (res.err) {
    console.log(`✗ [${TAG}] ${name}: renderWar HATA ${res.err}`);
    fail++;
    continue;
  }
  ...
  if (!ok) fail++;
  console.log(
    `${ok ? '✓' : '✗'} [${TAG}] ${name}: modal ${res.scroll ? 'KAYDIRIYOR' : 'sığıyor'} (${res.sh}/${res.ch})${res.page ? ' + SAYFA kayıyor' : ''}`,
  );
}

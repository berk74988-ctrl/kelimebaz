...
const server = createServer(async (req, res) => {
  try {
    ...
    if (p === '/' || p === '') p = '/index.html';
    let buf;
    try { buf = await readFile(join(ROOT, p)); } catch { buf = await readFile(join(ROOT, 'index.html')); p = '/index.html'; }
...
const validRaw = JSON.parse(await readFile(new URL('../src/app/data/valid-words-en.json', import.meta.url)));
const VALID_BY_LEN = {};
...
const isAiTurn = () => page.locator('.vs-turn.ai').isVisible().catch(() => false);
const resultUp = () => page.locator('.vs-result').isVisible().catch(() => false);
...
const playedBefore = await page.evaluate(() => { try { return JSON.parse(localStorage.getItem('kelimebaz:stats') || '{}').vsaiPlayed || 0; } catch { return 0; } });
...
const playedAfter = await page.evaluate(() => { try { return JSON.parse(localStorage.getItem('kelimebaz:stats') || '{}').vsaiPlayed || 0; } catch { return 0; } });
...
const ok =
  ...
  out.resultShown && out.hasVs === 2 && out.hasAgain && !!out.answerShown &&

...
const out = await page.evaluate(() => {
  ...
  try {
    // 1) Maliyet rebalance
    o.costs = {
      raider: ARMY_TYPES.raider.cost,
      axe: ARMY_TYPES.axe.cost,
      knight: ARMY_TYPES.knight.cost,
      archer: VV_UNITS.archer.cost,
      sword: VV_UNITS.sword.cost,
      guard: VV_UNITS.guard.cost,
    };
    // cost/dps ve cost/def oranları (denge)
    o.cpd = {
      raider: +(ARMY_TYPES.raider.cost / ARMY_TYPES.raider.dps).toFixed(1),

...
await page.evaluate(() => {
  ...
  const log = Array.from(
    { length: 8 },
    (_, i) =>
      `<div class="w-le">⚔️ <b>Oyuncu${i}</b> rakip köye saldırdı <span class="dmg">−${40 + i}</span></div>`,
  ).join('');
  document.getElementById('warBody').innerHTML = `
    ...
    <div class="w-log">${Array.from(
      { length: 6 },
      (_, i) =>
        `<div class="w-le">🗡️ <b>Akıncı${i}</b> · ${5 + i} gönderildi <span class="dmg">−${30 + i} HP</span> <span class="dmg">🏚️ ${i}</span> <span class="rep">💀 ${i}</span></div>`,
    ).join('')}</div>`;
});
...
console.log(
  `Savaş ekranı body kaydırma gerekiyor mu: ${warScroll.need ? 'EVET' : 'hayır'} (${warScroll.sh}>${warScroll.ch})`,
);
const pageScrollWar = await page.evaluate(
...
await page.evaluate(() => {
  const opp = Array.from(
    ...
    (_, i) =>
     ...
     <button class="cl-btn no">⚔️ Savaş Aç</button></div>`,
  ).join('');
  document.getElementById('warBody').innerHTML = `
...
await page.evaluate(() => {
  ...
  const army =
    btn('🗡️', 'Akıncı', 40, '1.0', '⚔️') +
    btn('🪓', 'Baltacı', 85, '2.0', '⚔️') +
    btn('🐎', 'Şövalye', 150, '3.5', '⚔️');
  const units =
    btn('🗡️', 'Akıncı', 40, '10', '🛡️') +
    btn('🪓', 'Baltacı', 85, '18', '🛡️') +
    btn('🐎', 'Şövalye', 150, '30', '🛡️');
  const builds =
    btn('🏹', 'Kule', 60, '120', '+') +
    btn('🧱', 'Sur', 40, '80', '+') +
    btn('🔥', 'Ocak', 90, '150', '+');
  ...
  const co = document.getElementById('vvCoins');
  if (co) co.textContent = '1250';
});
await page.waitForTimeout(300);
const dock = await page.evaluate(() => {
  const d = document.querySelector('#villageView .vv-dock');
  if (!d) return null;
  const rows = [...d.querySelectorAll('.vv-builds')].map((r) => ({
    wrap: r.scrollWidth > r.clientWidth + 1,
    h: r.offsetHeight,
  }));
  return {
    dockH: d.offsetHeight,
    vh: window.innerHeight,
    pct: Math.round((d.offsetHeight / window.innerHeight) * 100),
    rows,
  };
});
if (dock) {
  console.log(
    `Köy menüsü yüksekliği: ${dock.dockH}px (ekranın %${dock.pct}'i) · satırlar: ${dock.rows.map((r) => r.h + (r.wrap ? '(kaydırır)' : '')).join(', ')}`,
  );
}

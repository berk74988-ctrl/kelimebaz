...
for (const w of words) {
  const h = hints[w];
  if (!h || !h.h) { (h ? empty : missing).push(w); continue; }
...
if (missing.length) console.log('  EKSİK:', missing.slice(0, 30).join(' '), missing.length > 30 ? '…' : '');
if (leaks.length) { console.log('  SIZINTILI:'); for (const l of leaks) console.log('   ', l); }

...
const ANS = JSON.parse(
  await readFile(new URL('../src/app/data/words-en.json', import.meta.url)),
).words;
...
const CAT = [
  [
    'Animal',
    /\b(animal|mammal|bird|fish|insect|reptile|amphibian|creature|rodent|feline|canine|livestock|beast)\b/i,
  ],
  [
    'Food',
    /\b(food|fruit|vegetable|dish|meal|edible|snack|dessert|meat|grain|spice|drink|beverage|sauce|bread|cheese)\b/i,
  ],
  ...
  [
    'Person',
    /\b(person who|one who|someone who|a person|people who|worker|professional|expert in)\b/i,
  ],
  [
    'Place',
    /\b(place|area|region|building|location|country|city|town|room|structure|piece of land|ground)\b/i,
  ],
  ...
  [
    'Nature',
    /\b(weather|natural phenomenon|geological|celestial|mineral|rock|element|body of water)\b/i,
  ],
...
async function fetchWord(w, tries = 5) {
  for (let t = 0; t < tries; t++) {
    try {
      ...
      if (r.status === 429) {
        await new Promise((res) => setTimeout(res, 800 + t * 1200));
        continue;
      } // oran sınırı → bekle
...
async function worker() {
  while (idx < todo.length) {
    ...
    if (done % 150 === 0) {
      console.log(`  ${done}/${todo.length} denendi · toplam bulunan: ${found}`);
      await writeFile(OUT, JSON.stringify(out));
    }
  }
}
console.log(
  `İpucu üretiliyor: ${todo.length} eksik kelime (mevcut: ${Object.keys(out).length})...`,
);

...
for (const diff of ['easy', 'medium', 'hard']) {
  ...
  results[diff] = { avg: +(tot / Math.max(1, solved)).toFixed(2), winPct: Math.round((solved / MATCHES) * 100), dist };
}
...
for (const d of ['easy', 'medium', 'hard']) {
  const r = results[d];
  const dz = r.dist.slice(0, 6).map((n) => String(n).padStart(3)).join(' ') + '  X:' + r.dist[6];
  console.log(`${d.padEnd(8)} | ${String(TOPK[d]).padStart(3)}  | ${String(r.avg).padEnd(4)} | ${String(TARGET[d]).padEnd(5)} |   ${String(r.winPct).padStart(3)}%   | ${dz}`);
}
...
const ok = band('easy') && band('medium') && band('hard') &&
  results.easy.avg > results.medium.avg && results.medium.avg > results.hard.avg &&
  results.hard.winPct === 100 && worstTurnMs < 100;
console.log(ok

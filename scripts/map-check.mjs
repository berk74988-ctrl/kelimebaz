...
const setup = await page.evaluate(() => {
  ...
  try {
    ...
    document
      .querySelectorAll('.overlay.show,.vv.show,.auth-overlay.show')
    ...
    const R = (id, type, tier, x, y, gold, hp, owner, mine, tr, en) => ({
      ...
      hpBonus: hp,
      owner,
      ownerName: owner ? (mine ? 'Kartallar' : 'Kurtlar') : null,
      ownerLogo: owner ? (mine ? '🦅' : '🐺') : null,
      npc: !owner,
      hp: owner ? 300 : 220,
      max: owner ? 300 : 220,
      mine: !!mine,
      tr,
      en,
      defs: [{ t: 'wall' }, { t: 'wall' }, { t: 'tower' }],
      units: [
        { t: 'archer', x: -0.2, y: 0.3, lv: 1 },
        { t: 'sword', x: 0.2, y: 0.2, lv: 1 },
      ],
    });
    _map = {
      inClan: true,
      ...
      incomeRate: 22,
      totalRegions: 9,
      army: { raider: 6, axe: 3, knight: 2 },
      armyTypes: ARMY_TYPES,
      armyMax: 15,
      deployMax: 6,
      upg: {},
      regions: [
        R('plains', 'farm', 1, 0.16, 0.28, 8, 0, null, false, 'Yeşil Ovalar', 'Green Plains'),
        R('meadow', 'farm', 1, 0.3, 0.56, 9, 0, 'A', true, 'Çayırlık', 'Meadowlands'),
        ...
        R('quarry', 'mine', 2, 0.54, 0.8, 13, 0, null, false, 'Taş Ocağı', 'Stone Quarry'),
        R('fort_c', 'fortress', 3, 0.48, 0.44, 5, 60, 'A', true, 'Merkez Kale', 'Central Keep'),
        R('isle_gold', 'island', 2, 0.9, 0.82, 16, 20, null, false, 'Altın Ada', 'Gold Isle'),
...
const sel = await page.evaluate(() => {
  ...
  try {
    ...
    const p = mapToScreen(r.x * MAP_W, r.y * MAP_H);
    wmTap(p.x, p.y);
    ...
    const pp = mapToScreen(is.x * MAP_W, is.y * MAP_H);
    wmTap(pp.x, pp.y);

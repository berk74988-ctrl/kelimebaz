...
async function run(label, W, H, mobile) {
  ...
  o.layoutRow = await page.evaluate(
    () => getComputedStyle(document.querySelector('.cc-layout')).flexDirection,
  );
  o.joinRows = await page.locator('.cc-join-list .cl-row').count();
  o.createBtnInView = await page.evaluate(() => {
    const b = document.querySelector('.cc-create');
    if (!b) return false;
    const r = b.getBoundingClientRect();
    return r.top >= 0 && r.bottom <= innerHeight + 1;
  });
  o.bodyScrolls = await page.evaluate(() => {
    const s = document.getElementById('clanBody');
    return s.scrollHeight > s.clientHeight + 2;
  });
  o.dbg = await page.evaluate(() => {
    const s = document.getElementById('clanBody'),
      box = document.querySelector('.clan-box'),
      f = document.querySelector('.cc-col-form'),
      j = document.querySelector('.cc-col-join');
    return {
      sSH: s.scrollHeight,
      sCH: s.clientHeight,
...
console.log(
  ...
  JSON.stringify({
    row: desk.layoutRow,
    btnInView: desk.createBtnInView,
    scroll: desk.bodyScrolls,
    rows: desk.joinRows,
  }),
);
console.log(
  'mob:',
  JSON.stringify({ col: mob.layoutRow, rows: mob.joinRows, scroll: mob.bodyScrolls }),
);
console.log(

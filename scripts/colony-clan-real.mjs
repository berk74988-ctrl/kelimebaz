...
const mkMembers = (n) =>
  Array.from({ length: n }, (_, i) => ({
    user: 'u' + i,
    name: 'Oyuncu ' + i,
    role: i === 0 ? 'leader' : i === 1 ? 'coleader' : i === 2 ? 'commander' : 'member',
  }));
const SCEN = {
  yeni: {
    id: 'c1',
...
for (const [name, clan] of Object.entries(SCEN)) {
  ...
  if (res.err) {
    console.log(`✗ [${TAG}] ${name}: renderClanPage HATA ${res.err}`);
    fail++;
    continue;
  }
  ...
  if (!ok) fail++;
  console.log(
    `${ok ? '✓' : '✗'} [${TAG}] ${name}: klan paneli ${res.scroll ? 'KAYDIRIYOR' : 'sığıyor'} (${res.sh}/${res.ch})${res.page ? ' + SAYFA' : ''}`,
  );
}
await browser.close();
console.log(

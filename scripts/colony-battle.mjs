  ...
  try {
    ...
    _war = {
      ...
      foeVillage: {
        ...
        units: [
          { t: 'archer', x: 0.12, y: -0.1 },
          { t: 'sword', x: -0.2, y: 0.16 },
          { t: 'guard', x: 0.18, y: 0.22 },
        ],
      },
      war: {
        over: false,
        a: { clan: 'A', name: 'Kartallar', logo: '🦅', hp: 520, max: 1000, members: 5 },
...
const battle = await page.evaluate(() => {
  ...
  try {
    _rd.deploy = [
      { t: 'knight', x: 0, y: -1.4 },
      { t: 'knight', x: 0.35, y: -1.4 },
      { t: 'axe', x: 1.15, y: 0.7 },
      { t: 'axe', x: -1.15, y: 0.6 },
      { t: 'raider', x: 1.3, y: -0.4 },
      { t: 'raider', x: -1.3, y: -0.3 },
    ];
    _rd.sent = { knight: 2, axe: 2, raider: 2 };
    _rd.armyAtStart = { knight: 0, axe: 1, raider: 4 };
    _rdResult = {
      amt: 260,
      destroyed: ['wall', 'wall', 'wall', 'tower'],
      army: { knight: 2, axe: 2, raider: 5 },
      lost: 1,
      over: false,
      cdLeft: 45000,
    };
    ...
    let steps = 0;
    while (!_rdBattle.done && steps < 120) {
      raidStep(_rdBattle);
      rdCollectFx(_rdBattle);
      if (steps % 8 === 0) rdFxUpdate(0.05);
      steps++;
    }
    rdFxUpdate(0.016);
    rdHudUI();
...
const report = await page.evaluate(() => {
  ...
  try {
    renderBattleReport({
      ...
      stars: 2,
      razed: 3,
      lost: { raider: 1, axe: 0, knight: 0 },
      lostTot: 1,
      loot: { gold: 78, resKey: 'odun', resN: 62 },
      foe: _rd.foe,
    });

...
const out = await page.evaluate(() => {
  ...
  try {
    ...
    const S = raidState([{ t: 'raider', x: 0.5, y: 0 }], [], [{ t: 'archer', x: 0, y: 0, lv: 2 }], {
      raider: 3,
    });
    ...
    o.cost = {
      l1: upgCost('raider', 1).gold,
      l2: upgCost('raider', 2).gold,
      l3: upgCost('raider', 3).gold,
      l4: upgCost('raider', 4).gold,
    };
    ...
    openUpgrade();
    o.upgAboveVillage =
      getComputedStyle(document.getElementById('upgradeOverlay')).zIndex >
    ...
    _clanPost = (p, b) =>
      Promise.resolve({
        ok: true,
        d: {
          upg: Object.assign({}, _upg, { [b.type]: (_upg[b.type] || 1) + 1 }),
          type: b.type,
          level: (_upg[b.type] || 1) + 1,
        },
      });

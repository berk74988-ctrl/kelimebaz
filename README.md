...
- 🎨 **Tema Modu** — 8 tema (Mutfak · Doğa · Spor · Müzik · Sağlık · Ulaşım · Duygular · Tarih), her biri 40+ temalı kelime. Her temanın kendi ilerleme çubuğu; tamamlayınca altın, hepsini bitirince "Tema Ustası" rozeti. Casual (ligi etkilemez).
...
- 🎯 **Oyun tarzın** — tahmin geçmişinden çıkarılan kişisel içgörüler (harf körlüğü, uzunluk performansı, açılış alışkanlığı, tur verimliliği, ünlü/ünsüz dengesi). **Tüm analiz cihazda** — hiçbir veri dışarı gitmez. İsteğe bağlı "antrenman modu" serbest oyunu zayıf harflerine hafifçe kaydırır.
...
**Tema Modu** (`core/themes.ts` + `services/theme-mode.service.ts`) — 8 tema, kelime setleri `build-themes.mjs` ile **elle küratörlü** adaylardan üretilir: betik sözlüğe (valid-words) + 4-7 harfe göre süzer, her temada ≥40 kelime doğrular (`npm run build:themes`). Setler tembel iner (`themes-{tr,en}.json`). İlerleme cihazda (`kelimebaz:themes`, dil+tema başına bulunan kelimeler). Tema tamamlanınca altın **bir kez** ödenir (`claimed` seti), tümü bitince "Tema Ustası" rozeti (envanter). **Casual** (YZ modu gibi): lig/istatistik etkilenmez (`GameService.endGame`'de `casual = isVsai || isTheme`). Anlamsal gruplama LLM'le değil elle yapılır (heuristik semantik gruplayamaz); anahtar gelince `build-themes.mjs` LLM'le genişletebilir.

**Oyun tarzı analizi cihazda** (`core/play-style.ts` + `services/play-style.service.ts`) — oyuncunun son ~80 maçının tahmin geçmişi (cevap + tahminler) **kayan pencereyle** `kelimebaz:playstyle`'da tutulur (~10 KB). İçgörüler (harf körlüğü, uzunluk performansı, açılış, tur verimliliği, ünlü/ünsüz) **saf fonksiyonlarla** hesaplanır — LLM yok, ağ yok. **GİZLİLİK: hiçbir veri dışarı gönderilmez.** `MIN_GAMES` altında içgörü gösterilmez (yanıltıcı çıkarım yok, "biraz daha oyna"). İsteğe bağlı **antrenman modu** (ayarlardan, varsayılan kapalı) serbest modda kelime seçimini en zayıf harfe **~%35 olasılıkla** kaydırır — fark edilmeyecek kadar hafif.
